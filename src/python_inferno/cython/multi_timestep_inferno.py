# -*- coding: utf-8 -*-
import numpy as np

from ..configuration import npft
from ..precip_dry_day import precip_moving_sum
from .cython_inferno import multi_timestep_inferno


def cython_multi_timestep_inferno(
    *,
    t1p5m_tile,
    q1p5m_tile,
    pstar,
    sthu_soilt,
    frac,
    c_soil_dpm_gb,
    c_soil_rpm_gb,
    canht,
    ls_rain,
    con_rain,
    pop_den,
    flash_rate,
    ignition_method,
    fuel_build_up,
    fapar_diag_pft,
    dry_bal=None,
    dry_days,
    fapar_factor,
    fapar_centre,
    fuel_build_up_factor,
    fuel_build_up_centre,
    temperature_factor,
    temperature_centre,
    flammability_method,
    dryness_method,
    dry_day_factor,
    dry_day_centre,
    rain_f,
    vpd_f,
    dry_bal_factor,
    dry_bal_centre,
    timestep,
    return_dry_bal=False,
):
    if dry_bal is None:
        dry_bal = np.zeros_like(fapar_diag_pft)

    param_vars = dict(
        fapar_factor=fapar_factor,
        fapar_centre=fapar_centre,
        fuel_build_up_factor=fuel_build_up_factor,
        fuel_build_up_centre=fuel_build_up_centre,
        temperature_factor=temperature_factor,
        temperature_centre=temperature_centre,
        dry_day_factor=dry_day_factor,
        dry_day_centre=dry_day_centre,
        rain_f=rain_f,
        vpd_f=vpd_f,
        dry_bal_factor=dry_bal_factor,
        dry_bal_centre=dry_bal_centre,
    )

    # Ensure the parameters are given as arrays with 'npft' elements.
    transformed_param_vars = dict()
    for name, val in param_vars.items():
        if not hasattr(val, "__iter__"):
            print(f"Duplicating: {name}")
            val = [val] * npft
        transformed_param_vars[name] = np.asarray(val, dtype=np.float64)
        assert transformed_param_vars[name].shape == (npft,)

    cum_rain = precip_moving_sum(ls_rain=ls_rain, con_rain=con_rain, timestep=timestep)

    # Ensure consistency of the time dimension.
    if not (
        t1p5m_tile.shape[0]
        == q1p5m_tile.shape[0]
        == pstar.shape[0]
        == sthu_soilt.shape[0]
        == frac.shape[0]
        == c_soil_dpm_gb.shape[0]
        == c_soil_rpm_gb.shape[0]
        == canht.shape[0]
        == ls_rain.shape[0]
        == con_rain.shape[0]
        == fuel_build_up.shape[0]
        == fapar_diag_pft.shape[0]
        == cum_rain.shape[0]
    ):
        raise ValueError("All arrays need to have the same time dimension.")

    def float_multi_timestep_inferno(**kwargs):
        float_kwargs = dict()
        for name, val in kwargs.items():
            if hasattr(val, "__iter__"):
                float_kwargs[name] = np.asarray(val, dtype=np.float32)
            else:
                float_kwargs[name] = val
        return multi_timestep_inferno(**float_kwargs)

    ba, dry_bal = float_multi_timestep_inferno(
        t1p5m_tile=t1p5m_tile,
        q1p5m_tile=q1p5m_tile,
        pstar=pstar,
        sthu_soilt=sthu_soilt,
        frac=frac,
        c_soil_dpm_gb=c_soil_dpm_gb,
        c_soil_rpm_gb=c_soil_rpm_gb,
        canht=canht,
        ls_rain=ls_rain,
        con_rain=con_rain,
        # pop_den=pop_den,
        # flash_rate=flash_rate,
        ignition_method=ignition_method,
        fuel_build_up=fuel_build_up,
        fapar_diag_pft=fapar_diag_pft,
        dry_bal=dry_bal,
        dry_days=dry_days,
        flammability_method=flammability_method,
        dryness_method=dryness_method,
        cum_rain=cum_rain,
        **transformed_param_vars,
    )
    if return_dry_bal:
        return ba, dry_bal
    return ba
