...
kernel void multi_timestep_inferno(
    ...
    const int sthu_soilt_single_size = size_2d;
    const int frac_size = total_pft_size_3d;
    const int c_soil_dpm_gb_size = size_2d;
    const int c_soil_rpm_gb_size = size_2d;
    ...
    const device float* t1p5m_tile = in + method_params_size;
    const device float* q1p5m_tile = t1p5m_tile + t1p5m_tile_size;
    const device float* pstar = q1p5m_tile + q1p5m_tile_size;
    // XXX NOTE - This is with a single soil layer selected!
    const device float* sthu_soilt_single = pstar + pstar_size;
    const device float* frac = sthu_soilt_single + sthu_soilt_single_size;
    const device float* c_soil_dpm_gb = frac + frac_size;
    const device float* c_soil_rpm_gb = c_soil_dpm_gb + c_soil_dpm_gb_size;
